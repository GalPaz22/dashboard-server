...
const toolDefinitions = [
  ...
  },
  {
    ...
    description:
      ...
      "Scale the count to the result set: 1-5 for curated advice, 6-25 when many close matches should be shown as a normal results grid. " +
...
## איך לענות
פתח בהמלצה או בתשובה, לא בהקדמה. כשאתה מציג עד חמישה מוצרים, תן לכל מוצר את מה שהלקוח צריך כדי להחליט: שם, מחיר ומשפט קונקרטי למה הוא מתאים.

כשיש יותר מחמישה מוצרים שהם התאמות ישירות לבקשה, התשובה היא דף תוצאות ולא רשימת המלצות: כתוב רק משפט פתיחה קצר שמסכם מה נמצא, אל תכתוב רשימה ואל תתאר כל מוצר בטקסט. העבר את כל ההתאמות הרלוונטיות ל-\`present_products\` (עד 25). אם מוצר מסוים הוא התאמה חריגה או דורש הבהרה מועילה, הוסף עבורו בלבד משפט קצר ב-\`highlights\`; המשפט יוצג מתחת לכרטיס שלו.
...
- **בקשה ממוקדת** (תקציב, אירוע, סוג מוגדר — "יין אדום יבש למתנה עד 120") — שניים עד ארבעה מוצרים, ולכל אחד נימוק קצר למה דווקא הוא.
