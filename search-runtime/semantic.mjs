...
export const selectionSchema = objectSchema({matches:{type:'array',maxItems:20,items:objectSchema({
  id:str, evidence:{type:'array',items:objectSchema({requirement:{type:'integer'},field:{type:'string',enum:['title','categories','colors','finishes','tags','description']},quote:str})},
})}});
...
export function createSearchService(products, client, generate, {ttlMs=600000,maxEntries=100,maxCandidates=100,timeoutMs=30000,now=Date.now,lightweightRouter=true,routerTimeoutMs=10000}={}) {
  ...
  function neverEmptyFallback(query, plan, reason, metadata={}) {
    ...
    const sameType=visible.filter(p=>(!required.productType||p.productType===required.productType)&&(required.maxPrice===null||(p.price!==null&&p.price<=required.maxPrice)));
    const sameTags=sameType.filter(p=>required.tags.every(t=>(p.tags||[]).includes(t)));
    const pool=sameTags.length?sameTags:sameType.length?sameType:visible;
...
The active store context is: ${JSON.stringify({platform:client.platform,sourceUrl:client.sourceUrl,productTypes:client.productTypes,colors:client.colors,finishes:client.finishes,tagDefinitions:client.tagDefinitions,indexPlan:client.indexPlan})}
...
tagDefinitions are tenant-declared catalog attributes already verified against every product (e.g. "מסך מרובע" for a square screen); select any whose definition or synonyms match the request, even if the shopper's wording differs from its exact label. Only choose tag labels that appear in tagDefinitions above.
...
DATA ${JSON.stringify({query,detected:literal.plan,categories,colors:Object.keys(client.colors),finishes:Object.keys(client.finishes||{})})}`,plannerSchema);
      ...
      const colors=[...new Set([...required.colors,...plan.colors])], finishes=[...new Set([...required.finishes,...plan.finishes])], tags=[...new Set([...required.tags,...plan.tags])];
      const eligible=visible.filter(p=>(!required.productType||p.productType===required.productType)&&colors.every(c=>p.colors.includes(c))&&finishes.every(f=>p.finishes.includes(f))&&tags.every(t=>(p.tags||[]).includes(t))&&
      ...
      const scored=eligible.map(p=>({p,score:plan.categories.filter(c=>p.categories.includes(c)).length*4+terms.filter(t=>normalize(p.title).includes(t)).length*3})).filter(x=>x.score>0||tags.length)
        .sort((a,b)=>b.score-a.score||a.p.id.localeCompare(b.p.id));
      // Budgeted candidate set, never claim exhaustive semantic recall.
      const candidates=scored.slice(0,maxCandidates).map(({p})=>({id:p.id,title:p.title,categories:p.categories,colors:p.colors,finishes:p.finishes,tags:p.tags,description:(p.description||'').slice(0,2500),price:p.price}));
...
DATA ${JSON.stringify({query,requirements:plan.requirements,intent:plan.intent,candidates})}`,selectionSchema);
      ...
      for(const item of selection.matches){
        ...
        const covered=new Set();const valid=item.evidence.every(e=>{
          if(!e||!Number.isInteger(e.requirement)||e.requirement<0||e.requirement>=plan.requirements.length||!['title','categories','colors','finishes','tags','description'].includes(e.field)||typeof e.quote!=='string'||!e.quote.trim())return false;
