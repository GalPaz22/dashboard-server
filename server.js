...
async function getStoreConfigByApiKey (apiKey) {
  ...
  /* where you keep the users; change “semantix” if you chose
     another db name when you saved the record on onboarding   */
  const coreDb  = client.db("users");
  const userDoc = await coreDb
        .collection("users")
        .findOne({ apiKey });

  if (!userDoc) return null;

  /* what we expect was stored during onboarding */
  return {
    dbName  : userDoc.dbName,                      // mandatory
    products: userDoc.collections?.products || "products",
    queries : userDoc.collections?.queries  || "queries",
    /* optional – reuse later */
...
async function authenticate(req, res, next) {
  try {
    ...
    req.store = store;                // downstream routes just read this
    next();
  } catch (err) {
...
app.get("/autocomplete", async (req, res) => {
  const {
    query,                 // ⇠ search term
    apiKey,                  // ⇠ NEW : api-key from the plugin
    dbName : qDb,
...
async function translateQuery(query, context) {
  try {
    ...
    if (!needsTranslation) {
      // Optionally, you can still "clean" the query for embedding purposes.
      return query;
    }
...
app.post("/search", async (req, res) => {
  ...
  const { dbName, products: collectionName,
    categories: categories } = req.store;
console.log("categories", categories);



  /* the old guard still works – now it covers “missing key AND
     missing explicit db/collection names” */
